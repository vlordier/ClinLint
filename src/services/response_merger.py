def merge_responses(vale_results, llm_feedback):
    ...
    for issue in vale_results:
        if not isinstance(issue, dict) or not all(k in issue for k in ["Line", "Message", "Rule"]):
    ...
    summary = "No issues found." if not vale_results else f"{len(vale_results)} Vale issues detected; LLM provided suggestions."
